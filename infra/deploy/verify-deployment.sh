...
step "Log pipeline"
# WO-102: it waits for a freshly recreated promtail (up to 90 s) and says how
# long it waited — the first version fired at once, saw zero and rolled a
# good deploy back.
if COMPOSE="${COMPOSE}" "$(dirname "${BASH_SOURCE[0]}")/verify-log-pipeline.sh"; then
  :
else
  FAILURES=$((FAILURES + 1))
fi
