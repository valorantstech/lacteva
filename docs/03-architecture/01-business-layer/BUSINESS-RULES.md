...
**Clarification.** **Every table declares exactly one isolation strategy** — tenant-owned (a policy applies), platform-global (deliberately unprotected, with the reason on record), or mixed (both kinds of row, or isolation by a column other than `tenant_id`). "Nobody decided" is not an option, and a table in none of the three fails the build. Every tenant-owned table carries a PostgreSQL row-level security policy comparing its `tenant_id` against a transaction-scoped session setting, so a query that forgets its filter returns NOTHING rather than another tenant's data. The application's own `tenant_id` predicates remain in place and become defense-in-depth: correctness no longer depends on every future query remembering them. The policy covers reads, updates, and deletes through `USING`, and writes through `WITH CHECK` — without the latter a caller could move a row INTO another tenant, succeeding silently. `FORCE` is mandatory because the application connects as the table owner, which would otherwise bypass its own policies. An unbound session matches nothing and therefore fails CLOSED. Rows that belong to no tenant (`tenant_id IS NULL` — user accounts before they join an organization, the role catalog, the outbox log) are visible to everyone by design, and the policy must say so explicitly: SQL three-valued logic makes `NULL = 'x'` neither true nor false, so a predicate that omits the NULL case hides platform-global rows from every session including the one that owns them — which silently breaks registration itself. Cross-tenant machinery (relay dispatch, consumers, projection rebuilds, platform administration) **must** set an explicit, transaction-scoped, logged bypass — never a superuser connection, and never nothing at all. MT-001 found that they had been setting nothing: an unbound session matches no tenant-owned row, so the entire asynchronous half of the platform could see only platform-global data and every projection write was refused, without an error anywhere. **Inside a bypassed session the application-level `tenant_id` filter is the only isolation there is**, so every query in that context filters explicitly, even where a UUID makes a collision implausible. **And a tenant boundary is not only a database boundary:** any key namespace derived from an identifier that is unique *per tenant* rather than globally — a Redis rate-limit key built from an email address, an object-storage prefix — must carry the tenant too, or two tenants share one counter and one bucket path. A new tenant-owned table without a policy is a build failure, not a latent leak. Two consequences follow that the original rule did not state. First, a table that is tenant-owned but reachable only through a parent — a settlement line, a supplier profile — is still tenant-owned; SEC-002 gave the thirteen such tables their own `tenant_id` rather than leaving them to whichever query remembered the join. Second, **the tenant must be bound before the first tenant-owned read, not after it.** Authentication is a chicken-and-egg problem under RLS — the session row proving who you are is itself tenant-owned — and the resolution is to bind from an authoritative source (a signed token claim, a single bypassed lookup by an unguessable token) before touching any row. A `set_current_tenant()` not paired with a re-bind moves the context and leaves the database behind.

**Enforcement.** `core/rls.py` — `bind_tenant()` (`SET LOCAL`, so a pooled connection cannot carry a tenant across requests), `rebind_tenant()`, `bind_platform_context()`, `platform_session()` / `PlatformSessionFactory` (the bypass boundary — a component built with it cannot forget), `policy_statements()`, `identity_policy_statements()`, and the `PLATFORM_GLOBAL`/`MIXED` registers with `unclassified_tables()`; `core/db.py` — per-request binding; `api/deps.py` — binding from the token claim BEFORE the first read; migrations `a1c7f3b90e22` (the policy set), `c94b1ea27f31` (the platform-global clause), `f2d18ba60c47` (13 child tables plus `organization`).

**Verification.** `test_security.py::test_every_tenant_owned_table_is_covered_by_a_policy`, `::test_the_rls_policy_denies_by_default_and_checks_writes`, `::test_application_level_tenant_isolation_holds`; `test_rls_postgres.py::test_a_query_that_forgets_its_filter_still_cannot_leak`, `::test_cross_tenant_update_affects_nothing`, `::test_cross_tenant_delete_affects_nothing`, `::test_a_row_cannot_be_written_into_another_tenant`, `::test_no_tenant_bound_means_no_rows`, `::test_platform_global_rows_are_visible_to_everyone`, `::test_a_tenant_still_cannot_see_another_tenants_rows`, `::test_every_tenant_owned_table_is_enabled_and_forced`, `::test_every_tenant_owned_table_has_using_and_with_check`, `::test_a_tenant_sees_only_its_own_organization`, `::test_settlement_lines_do_not_leak_across_tenants`, `::test_supplier_pii_does_not_leak_across_tenants` (the PostgreSQL Verification workflow — a skip is a collection error there, not a skip); `test_security.py::test_every_table_declares_an_isolation_strategy`, `::test_authentication_binds_the_tenant_before_reading_any_row`; `test_multitenancy.py::test_background_components_receive_a_platform_bound_session_factory`, `::test_a_per_identifier_budget_is_namespaced_by_tenant`, `::test_a_notification_never_resolves_another_tenants_recipient`, `::test_object_keys_are_prefixed_by_tenant`; `test_rls_postgres.py::test_an_unbound_background_session_sees_no_tenant_events`.

**Status:** Active (SEC-001; platform-global clause CI-001; coverage, identity isolation and binding order SEC-002; the bypass boundary and non-database key namespaces MT-001 — the machinery that was documented as bypassing had never actually been doing so).
