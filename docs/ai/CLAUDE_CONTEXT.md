...
- **Current platform / product:** Procurement — the collection→pricing→settlement→payment→receipt lifecycle is complete end to end; offline collection is the last Phase A item before Collect is field-ready.
...
- **Parallel debt queue:** offline sync engine for Flutter (Collect prerequisite); a production SMS/email provider adapter and per-tenant template overrides (NOT-001 debt); money-precision policy (values are `Float` today by documented decision — the calculation increment must define `Numeric`/rounding); governance ratification (MR-1/MR-3) and founding ADR backfill (B4).
