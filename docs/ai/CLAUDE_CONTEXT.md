...
- **Current platform / product:** Procurement is COMPLETE end to end — collection (online or offline) → pricing → settlement → payment → receipt, with notifications throughout. Phase A of the execution plan is finished; **Phase B (production hardening) is next**.
...
| 9 | Platform debt markers: invitation token still returned in the API response (the invitation *notification* now exists, but no provider transmits it), per-request permission DB queries (Redis cache pending), HS256 JWTs, no Postgres RLS, supplier document upload requires MinIO even in dev-lite. All marked `TODO(M#)` in code. *(Consumer framework: resolved by SPRINT-008B.)* | Open, tracked. |
