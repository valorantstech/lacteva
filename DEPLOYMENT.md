...
**A release directory that is live is never deleted or re-staged in place.** `current` names one release directory, and every infrastructure container (nginx, promtail, Loki, Grafana, Prometheus, postgres's backup and init scripts) bind-mounts its configuration out of it. Staging therefore builds the new tree in a sibling `releases/.staging-<tag>-<pid>` directory, checks it is complete, and only then swaps it in with two renames. If the directory being replaced is live — `current` points at it, or a running container mounts anything under it — it is **retired by rename** to `releases/.retired-<tag>-<stamp>` (running containers keep their mounted inodes) rather than deleted, and retired trees are removed only at the end of a fully verified deploy, and only once nothing mounts them. This exists because on 2026-09-26 a re-deploy of the tag that `current` already named (the half-rollback above had left it there) began with `rm -rf` of that directory and died before refilling it: production ran for an hour on configuration files that no longer existed, and the next nginx reload would have taken every site down. Every `rm -rf` in `deploy.sh` is now either of a staging directory the run owns or guarded by that live check, and a test pins it.
