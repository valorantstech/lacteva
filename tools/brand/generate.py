...
def _dart_mark() -> str:
    ...
    caps = mark.wordmark_caps_bbox()
    gradient = wordmark["gradient"]
    reference = wordmark["reference"]
...
// ---------------------------------------------------------------------------
// The traced wordmark (LACTEVA-BRAND-004 Amendment 1)
// ---------------------------------------------------------------------------
//
// The owner's letterforms, vectorized from the binding reference. They arrive
// as PATH DATA rather than as `..cubicTo` chains because the four layers
// expand to some sixty kilobytes of Dart that way; `lactevaPathData` in
// mark.dart walks them, and a parser of four commands is both smaller and
// easier to check than sixty kilobytes of generated call sites.
//
// Amendment 1 is why any of this exists: no committed surface may carry a
// font-rendered approximation of LACTEVA, and the splash used to set the word
// in whatever the UI font happened to be.
//
// All of it is in the artwork's own {reference["width"]} x {reference["height"]} space.

/// LACTE.
const String kWordmarkNavyData =
    '{mark.wordmark_layer("navy")}';

/// VA, with the drop seated in the final A.
const String kWordmarkGreenData =
    '{mark.wordmark_layer("green")}';

/// The tagline's two flanking rules.
const String kWordmarkRuleData =
    '{mark.wordmark_layer("rule")}';

/// The tagline itself, drawn rather than set.
const String kWordmarkTaglineData =
    '{mark.wordmark_layer("tagline")}';

/// The LACTEVA caps alone — both letter layers, tagline excluded.
const Rect kWordmarkCapsBounds = Rect.fromLTWH(
  {n(caps[0])},
  {n(caps[1])},
  {n(caps[2])},
  {n(caps[3])},
...
/// The whole artwork, caps and tagline together.
const Rect kWordmarkBounds = Rect.fromLTWH(
  0,
  0,
  {n(float(reference["width"]))},
  {n(float(reference["height"]))},
);

/// The words, so no surface retypes them and gets the punctuation wrong.
const String kTagline = '{mark.TAGLINE}';

// The LOGO's own colours. Brand data, NOT Design System tokens — WO-31 is
// explicit that no navy enters a token, a component or a theme, so they live
// here beside the geometry they belong to and nowhere else.
const int kLogoNavy = 0xFF{channel(mark.LOGO_NAVY)};
const int kLogoVaTop = 0xFF{channel(mark.LOGO_VA_TOP)};
const int kLogoVaBottom = 0xFF{channel(mark.LOGO_VA_BOTTOM)};
const int kLogoRule = 0xFF{channel(mark.LOGO_RULE)};
const int kLogoTaglineInk = 0xFF{channel(mark.LOGO_TAGLINE)};
const int kLogoCream = 0xFF{channel(mark.LOGO_CREAM)};
const int kLogoDeep = 0xFF{channel(mark.LOGO_DEEP)};
const int kLogoDairy = 0xFF{channel(mark.LOGO_DAIRY)};

/// Where the VA gradient runs, in the artwork's own coordinates. Vertical:
/// a horizontal fit was tried against the reference and does not describe it.
const double kWordmarkGradientTop = {n(float(gradient["y0"]))};
const double kWordmarkGradientBottom = {n(float(gradient["y1"]))};
'''
