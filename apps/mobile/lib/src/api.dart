...
class ApiClient {
  ...
  /// Push a batch of operations captured offline (OFF-001). Each carries its
  /// own idempotency key, so re-sending after a lost acknowledgement is safe.
  Future<Map<String, dynamic>> pushSyncBatch({
    ...
    final result =
        await _send(
              'POST',
              '/v1/sync/collection',
              body: {'device_id': deviceId, 'operations': operations},
            )
            as Map<String, dynamic>;
    return result;
  }

  /// Receipt history (RCP-001). Receipts are generated by the platform from
  /// completed payments; the app only reads and previews them.
  Future<ReceiptPageResult> listReceipts({
    ...
    final qs = Uri(queryParameters: params).query;
    final result =
        await _send('GET', '/v1/receipts?$qs') as Map<String, dynamic>;
    return ReceiptPageResult.fromJson(result);
  }

  Future<ReceiptDetailResult> receiptDetail(String id) async {
    final result =
        await _send('GET', '/v1/receipts/$id') as Map<String, dynamic>;
    return ReceiptDetailResult.fromJson(result);
  }

  /// Render an artifact (json | html | pdf). PDF is a platform placeholder.
  Future<RenderedReceipt> renderReceipt(String id, String format) async {
    final result =
        await _send('GET', '/v1/receipts/$id/render?format=$format')
            as Map<String, dynamic>;
    return RenderedReceipt.fromJson(result);
  }
...
class ReceiptSummary {
  ...
  factory ReceiptSummary.fromJson(Map<String, dynamic> json) => ReceiptSummary(
    id: json['id'] as String,
    number: json['receipt_number'] as String,
...
class ReceiptPageResult {
  ReceiptPageResult({required this.items, required this.total});

  factory ReceiptPageResult.fromJson(Map<String, dynamic> json) =>
      ReceiptPageResult(
        items: (json['items'] as List<dynamic>)
...
class ReceiptLineSummary {
  ...
  factory ReceiptLineSummary.fromJson(Map<String, dynamic> json) =>
      ReceiptLineSummary(
        settlementNumber: json['settlement_number'] as String,
        grossAmount: json['gross_amount'].toString(),
...
class ReceiptDetailResult {
  ...
  factory ReceiptDetailResult.fromJson(Map<String, dynamic> json) =>
      ReceiptDetailResult(
        receipt: ReceiptSummary.fromJson(
          json['receipt'] as Map<String, dynamic>,
        ),
        lines: (json['lines'] as List<dynamic>)
            .map((e) => ReceiptLineSummary.fromJson(e as Map<String, dynamic>))
            .toList(),
        availableFormats:
            ((json['metadata'] as Map<String, dynamic>)['available_formats']
                    as List<dynamic>)
                .map((e) => e as String)
                .toList(),
      );
  ...
  final List<ReceiptLineSummary> lines;
...
class RenderedReceipt {
  ...
  factory RenderedReceipt.fromJson(Map<String, dynamic> json) =>
      RenderedReceipt(
