...
import { ApiError, type ReceivableRow, type ReceivablesPage, getReceivables } from "@/lib/api";
...
export default function ReceivablesPage_() {
  ...
  const columns: Column<ReceivableRow>[] = [
    {
      ...
      cell: (row) => (
        <div className="flex flex-col">
          <Link className="font-medium hover:underline" href={`/customers/${row.customer_id}`}>
            {row.name}
          </Link>
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex flex-col items-end">
          ...
          {row.open_invoices ? (
            <span className="text-xs text-muted-foreground">
              {row.open_invoices} open {row.open_invoices === 1 ? "bill" : "bills"}
            </span>
          ) : null}
        </div>
      ),
    ...
    },
    {
      ...
      cell: (row) =>
        day(row.last_payment_at) ? (
          <span className="tabular-nums text-sm">{day(row.last_payment_at)}</span>
        ) : (
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 p-4 sm:p-6 lg:p-8">
      <PageHeader
        breadcrumbs={[{ label: "Dashboard", href: "/" }, { label: "Receivables" }]}
      ...
      >
        <StatTile
          ...
          hint="across every customer matching this filter"
          icon={<Wallet className="size-4" />}
        />
        <StatTile
          label={owingOnly ? "Customers owing" : "Customers"}
          value={page ? page.total : "—"}
          hint={owingOnly ? "with a balance above zero" : "including settled accounts"}
      ...
      <p className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        Open a customer to see their bills and record what they pay. A receipt is generated by the
        platform from the payment.
      </p>
    </div>
  );
}
