 ...
 *
 * TWO filters, composed: permission first, then module (D-31 / WO-85). An
 * entry is offered when the session may use it AND its organisation runs
 * the module it belongs to. An organisation with both modules — every one
 * that existed before D-31 — sees exactly what it saw before; a milk shop
 * (`["sales"]`) is not offered Settlements or Rate cards; a classic dairy
 * (`["collection"]`) is not offered Customers or Deliveries.
 *
 * Hiding is not security, and must not be mistaken for it — the same
 * argument PERM-001 makes about the permission filter, and it holds for the
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    getSession()
      ...
      .catch(
        () =>
          !cancelled && setSession({ authenticated: false, unreachable: true }),
      )
  ...
  const signedIn =
    checked && session?.authenticated === true && pathname !== "/login";
  ...
  const t = translatorFor(locale, { salesOnly: salesOnly(session) });
...
/** Tests only: the registry the menu is built from, with each entry's module,
 *  and the filter that decides what a session is offered — so a test asserts
